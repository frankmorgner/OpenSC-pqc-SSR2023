...
#define DILITHIUM_SECRETKEYBYTES (2*SEEDBYTES + CRHBYTES \
                               + L*POLYETA_PACKEDBYTES \
                               + K*POLYETA_PACKEDBYTES \
                               + K*POLYT0_PACKEDBYTES)
#define DILITHIUM_BYTES (L*POLYZ_PACKEDBYTES + OMEGA + K + N/8 + 8)
...
static int test_dilithium(void)
{
  ...
  size_t mlen, smlen;
  uint8_t m[MLEN];
  uint8_t sm[MLEN + DILITHIUM_BYTES];
  uint8_t m2[MLEN + DILITHIUM_BYTES];
...
static int test_dilithium_card(sc_card_t *card)
{
#if 0
	uint8_t sm[MLEN + DILITHIUM_BYTES];
	uint8_t pk[DILITHIUM_PUBLICKEYBYTES + 10];
	uint8_t sk[DILITHIUM_SECRETKEYBYTES + 10];

	sc_apdu_t apdu;
	sc_format_apdu_ex(&apdu, 0x00, 0x02, 0x01, 0x00, NULL, 0, sk, sizeof sk);
	sc_transmit_apdu(card, &apdu);
	if (apdu.resplen != DILITHIUM_SECRETKEYBYTES) {
		printf("Dilithium SK wrong length\n");
		return -1;
	}

	sc_format_apdu_ex(&apdu, 0x00, 0x02, 0x02, 0x00, NULL, 0, pk, sizeof pk);
	sc_transmit_apdu(card, &apdu);
	if (apdu.resplen != DILITHIUM_PUBLICKEYBYTES) {
		printf("Dilithium PK wrong length\n");
		return -1;
	}

	sc_format_apdu_ex(&apdu, 0x00, 0x02, 0x03, 0x00, NULL, 0, sm, sizeof sm);
	sc_transmit_apdu(card, &apdu);
	if (apdu.resplen != DILITHIUM_BYTES) {
		printf("Dilithium sig wrong length\n");
		return -1;
	}
	if (crypto_sign_verify(sm, DILITHIUM_BYTES, pk, DILITHIUM_PUBLICKEYBYTES, pk)) {
		printf("Dilithium sig bad\n");
		return -1;
	}

	size_t smlen = sizeof sm;
	if (crypto_sign_signature(sm, &smlen, sk, DILITHIUM_SECRETKEYBYTES, sk)) {
		printf("Dilithium sig create fail\n");
		return -1;
	}
	sc_format_apdu_ex(&apdu, 0x00, 0x02, 0x04, 0x00, sm, smlen, NULL, 0);
	sc_transmit_apdu(card, &apdu);
	if (apdu.sw1 != 0x90 || apdu.sw2 != 0x00) {
		printf("Dilithium verify fail\n");
		return -1;
	}

	printf("No dilithium card error\n");
#endif
	return 0;
}

#ifdef ENABLE_KYBER_REFERENCE
#undef API_H
#undef PARAMS_H
...
static int test_keys()
{
  ...
  if(memcmp(key_a, key_b, KYBER_SSBYTES))
	  printf("ERROR kyber keys\n");
  else
	  printf("No kyber error\n");
...
#define KYBER_CIPHERTEXTBYTES PQCLEAN_KYBER1024_CLEAN_CRYPTO_CIPHERTEXTBYTES
#define KYBER_SECRETKEYBYTES PQCLEAN_KYBER1024_CLEAN_CRYPTO_SECRETKEYBYTES
#define KYBER_PUBLICKEYBYTES PQCLEAN_KYBER1024_CLEAN_CRYPTO_PUBLICKEYBYTES
#define KYBER_SSBYTES PQCLEAN_KYBER1024_CLEAN_CRYPTO_BYTES
...
static int test_keys()
{
  ...
  //Bob derives a secret key and creates a response
  PQCLEAN_KYBER1024_CLEAN_crypto_kem_enc(ct, key_b, pk);
...
#define SFID_EF_CARDPQC 0x1A
static int get_ef_card_access_pqc(sc_card_t *card,
		u8 **ef_cardaccess, size_t *length_ef_cardaccess)
{
	return iso7816_read_binary_sfid(card, SFID_EF_CARDPQC, ef_cardaccess, length_ef_cardaccess);
}
 ...
 * @param[in] ct ciphertext from key encapsulation
 ...
 */
static BUF_MEM *
get_ta_sigdata(const BUF_MEM *id_picc, const BUF_MEM *nonce,
        const unsigned char ct[KYBER_CIPHERTEXTBYTES], const BUF_MEM *auxdata)
{
    size_t len;
    BUF_MEM *data = NULL;
    ...
    /* Data to be signed: ID PICC || r PICC || ct PCD || APCD */

    /* Authenticated auxiliary data are optional (only necessary if special
     * functions will be used later on) */
    if (auxdata) {
        len = id_picc->length + nonce->length + KYBER_CIPHERTEXTBYTES + auxdata->length;
    } else {
        len = id_picc->length + nonce->length + KYBER_CIPHERTEXTBYTES;
    }
    ...
    memcpy(data->data + id_picc->length + nonce->length, ct, KYBER_CIPHERTEXTBYTES);
    if (auxdata)
        memcpy(data->data + id_picc->length + nonce->length + KYBER_CIPHERTEXTBYTES,
			   	auxdata->data, auxdata->length);
		...
		const unsigned char *auxiliary_data, size_t auxiliary_data_len)
{
	...
	size_t cert_len = 0, ef_cardaccess_length = 0, ef_cardaccess_pqc_length = 0;
	...
	unsigned char *ef_cardaccess_pqc = NULL;
	EAC_CTX *eac_ctx = NULL;
...
#define ASC_id_TA_DILITHIUM		ASC_id_TA".3"
#define ASC_id_TA_DILITHIUM_3		ASC_id_TA_DILITHIUM".3"
#define SN_id_TA_DILITHIUM_3 "id-TA-DILITHIUM-3"
	OBJ_create(ASC_id_TA_DILITHIUM_3, SN_id_TA_DILITHIUM_3, SN_id_TA_DILITHIUM_3);

	ASN1_OBJECT *dilithium3 = OBJ_txt2obj(ASC_id_TA_DILITHIUM_3, 1);
	if (dilithium3 && 0 == OBJ_cmp(cvc_cert->body->public_key->oid, dilithium3)) {
		do_dilithium = 1;
	}

	if (do_dilithium) {
		/* get encapsulation public key */
		r = get_ef_card_access_pqc(card, &ef_cardaccess_pqc, &ef_cardaccess_pqc_length);
		if (r < 0) {
			sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Could not get EF.CardAccessPQC.");
			goto err;
		}
		if (ef_cardaccess_pqc_length != (size_t) KYBER_PUBLICKEYBYTES) {
			goto err;
		}

		/* Generate and encapsulate Key for CA */
		if (eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret)
			BUF_MEM_free(eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret);
		eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret = BUF_MEM_create(KYBER_SSBYTES);
		if (eacsmctx->eph_pub_key)
			BUF_MEM_free(eacsmctx->eph_pub_key);
		eacsmctx->eph_pub_key = BUF_MEM_create(KYBER_CIPHERTEXTBYTES);
		if (ef_cardaccess_pqc_length != (size_t) KYBER_PUBLICKEYBYTES
				|| !eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret
				|| !eacsmctx->eph_pub_key) {
			sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Could not get EF.CardAccessPQC.");
			goto err;
		}
#ifdef ENABLE_KYBER_REFERENCE
		crypto_kem_enc((unsigned char *) eacsmctx->eph_pub_key->data,
				(unsigned char *) eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret->data,
				ef_cardaccess_pqc);
#elif ENABLE_PQCLEAN
		PQCLEAN_KYBER1024_CLEAN_crypto_kem_enc((unsigned char *) eacsmctx->eph_pub_key->data,
				(unsigned char *) eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret->data,
				ef_cardaccess_pqc);
#endif
		sc_debug_hex(card->ctx, 1, "shared secret", (unsigned char *) eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret->data, eacsmctx->ctx->ca_ctx->ka_ctx->shared_secret->length);
		sc_debug_hex(card->ctx, 1, "kyber pub", (unsigned char *) ef_cardaccess_pqc, ef_cardaccess_pqc_length);
		sc_debug_hex(card->ctx, 1, "kyber ciphertext", (unsigned char *) (unsigned char *) eacsmctx->eph_pub_key->data, eacsmctx->eph_pub_key->length);

		r = eac_mse_set_at_ta(card, OBJ_obj2nid(dilithium3), chr, chr_len,
	...
	if (do_dilithium) {
		...
		BUF_MEM *data = get_ta_sigdata(eacsmctx->id_icc, nonce,
				(unsigned char *) eacsmctx->eph_pub_key->data, eacsmctx->auxiliary_data);

		signature = BUF_MEM_create(DILITHIUM_BYTES);
		if (signature && data && privkey_len == DILITHIUM_SECRETKEYBYTES) {
			crypto_sign_signature((unsigned char *) signature->data, &signature->length, (unsigned char *) data->data, data->length, privkey);
			BUF_MEM_free(data);
		} else {
...
#define ASC_id_CA_KYBER		ASC_id_CA".3"
#define ASC_id_CA_KYBER_K4		ASC_id_CA_KYBER".4"
#define ASC_id_CA_KYBER_K4_AES_CBC_CMAC_256		ASC_id_CA_KYBER_K4".4"
#define SN_id_CA_KYBER_K4_AES_CBC_CMAC_256 "id-CA-KYBER-K4-AES-CBC-CMAC-256"
		OBJ_create(ASC_id_CA_KYBER_K4_AES_CBC_CMAC_256, SN_id_CA_KYBER_K4_AES_CBC_CMAC_256, SN_id_CA_KYBER_K4_AES_CBC_CMAC_256);
		ASN1_OBJECT *kyber4 = OBJ_txt2obj(ASC_id_CA_KYBER_K4_AES_CBC_CMAC_256, 1);
		r = eac_mse_set_at_ca(card, OBJ_obj2nid(kyber4));
		if (r < 0) {
			sc_debug(card->ctx, SC_LOG_DEBUG_VERBOSE, "Could not select protocol properties "
					"(MSE: Set AT failed).");
			goto err;
        ...
        ctx->ca_ctx->ka_ctx->mac_keylen = 32;
        ctx->ca_ctx->ka_ctx->md = EVP_sha256();
        ctx->ca_ctx->ka_ctx->cipher = EVP_aes_256_cbc();
        ctx->ca_ctx->ka_ctx->enc_keylen = EVP_CIPHER_key_length(ctx->ca_ctx->ka_ctx->cipher);

		eph_pub_key = BUF_MEM_create_init(picc_pubkey,
				picc_pubkey_len);
	}
	...
	if (do_dilithium == 0) {
		...
		/* Trigger KDF calculation and ignore result. Suppress OpenPACE's
		 * error, for a bad authentication token. */
		int bak, new;
		fflush(stderr);
		bak = dup(2);
		...
		CA_STEP6_derive_keys(ctx, nonce, token);

		fflush(stderr);
		dup2(bak, 2);
		close(bak);

		sc_debug_hex(card->ctx, 1, "k mac", (unsigned char *) ctx->ca_ctx->ka_ctx->k_mac->data, ctx->ca_ctx->ka_ctx->k_mac->length);
		sc_debug_hex(card->ctx, 1, "k enc", (unsigned char *) ctx->ca_ctx->ka_ctx->k_enc->data, ctx->ca_ctx->ka_ctx->k_enc->length);
	}
