...
#define ASC_id_CA_KYBER_4		ASC_id_CA_KYBER".4"
#define ASC_id_CA_KYBER_4_AES_CBC_CMAC_256		ASC_id_CA_KYBER_4".4"
...
#define NTESTS 10
        ...
        if (!cert->body->certificate_authority_reference
                || !ASN1_OCTET_STRING_set(cert->body->certificate_authority_reference,
                    (unsigned char *) "ZZPQCCVCA", 9))
            goto pqcerr;
        ...
        if (!cert->body->public_key)
            goto pqcerr;
        cert->body->public_key->oid = OBJ_txt2obj(ASC_id_TA_DILITHIUM_3, 1);
        cert->body->public_key->cont1 = ASN1_UTF8STRING_new();
        if (!cert->body->public_key->cont1
                || !ASN1_OCTET_STRING_set(cert->body->public_key->cont1,
                    pk, sizeof pk))
            goto pqcerr;

        /* write effective date 02.12.2020 */
        ...
        if (!cert->body->certificate_effective_date
                || !ASN1_OCTET_STRING_set(cert->body->certificate_effective_date,
                    (unsigned char *) "201202", 6))
            goto pqcerr;

        /* write expiration date 02.12.2030 */
        ...
        if (!cert->body->certificate_expiration_date
                || !ASN1_OCTET_STRING_set(cert->body->certificate_expiration_date,
                    (unsigned char *) "301202", 6))
            goto pqcerr;

        /* write chat */
        cert->body->chat = CVC_CHAT_new();
        if (!cert->body->chat)
            goto pqcerr;
        cert->body->chat->terminal_type = OBJ_nid2obj(NID_id_AT);
        unsigned char authorization[EAC_AT_CHAT_BYTES];
        /* Role: CVCA, all permission bits set */
        memset(authorization, 0xFF, sizeof authorization);
        cert->body->chat->relative_authorization = ASN1_OCTET_STRING_new();
        if (!cert->body->chat->relative_authorization
                || !ASN1_OCTET_STRING_set(cert->body->chat->relative_authorization,
                    authorization, sizeof authorization))
            goto pqcerr;

        /* sign body */
        len = i2d_CVC_CERT_BODY(cert->body, &buf);
        if (len <= 0)
            goto pqcerr;
        crypto_sign_signature(sm, &smlen, buf, len, sk);
        OPENSSL_free(buf);
        buf = NULL;
        ...
        if (!cert->signature
                /* OpenSC can't handle more than 2532 bytes for this, why? */
                || !ASN1_OCTET_STRING_set(cert->signature,
                    sm, smlen))
            goto pqcerr;

        /* write certificate */
        len = i2d_CVC_CERT(cert, &buf);
        if (len <= 0)
            goto pqcerr;
        if (0 != write_file("ZZPQCCVCA.cvcert", buf, len))
            goto pqcerr;
        printf("Created %s\n", "ZZPQCCVCA.cvcert");
        OPENSSL_free(buf);
        buf = NULL;

        if (0 != write_file("ZZPQCCVCA.dilithium3", sk, sizeof sk))
            goto pqcerr;
        printf("Created %s\n", "ZZPQCCVCA.dilithium3");
...
pqcerr:
        ...
        return fail;
#else
        fprintf(stderr, "OpenPACE missing\n");
        exit(1);
#endif
    }
